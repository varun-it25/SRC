  ...
  return (
    <Container>
      <div className="w-full flex-col flex h-fit p-6 space-y-6">
        ...
        {loading ? (
          <div className="flex justify-center items-center h-full space-x-4"><UploadCloud size={40} className="animate-spin" /><p>Uploading...</p></div>
        ) : status === 'success' ? (
          <div className="w-full bg-green-200 text-center p-4 rounded-lg"><p className="font-semibold text-xl">Event Created Successfully!</p></div>
        ) : status === 'error' ? (
          <div className="w-full bg-red-200 text-center p-4 rounded-lg"><p className="font-semibold text-xl">{errorMessage}</p></div>
        ) : (
          <>
            ...
            <Button className="flex space-x-2" onClick={handleSubmit} disabled={!isFormValid}>
              <UploadCloud size={30} /><p>Publish</p>
            </Button>
          </>
        )}
      </div>
